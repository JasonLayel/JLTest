...
delivered to Jason. This applies to all rendered copy, generated marketing text (captions,
listings, posts), titles, alt/aria text, code comments, commit messages,
and chat replies. Jason considers the long dash a telltale sign of AI
writing and wants it gone completely.
...
the long dashes are banned. When in doubt, rewrite so no dash is needed.
...
- `advisor/` + `.claude/` (this branch): an art-career advisor agent, its
  `/advisor` and `/publish-kit` commands, and the memory files the advisor
  maintains.

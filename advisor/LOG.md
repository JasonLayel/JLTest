...
## 2026-08-04: Standing rule set, no long dashes, ever

Jason gave a permanent instruction: never deliver anything containing an
em-dash or en-dash (he considers the long dash an AI tell). Recorded it in
a root CLAUDE.md (both working branches), the advisor agent prompt, and the
publish-kit voice rules. Stripped every existing long dash from the live
site and from these memory files, replacing with commas, colons, or
separate sentences. Also brightened the hero image (lighter overlay plus a
small brightness lift) and let it show its full height. Apply this rule to
all future copy, commits, and chat replies.
...
the site: domain purchase, 10 to 15 curated pieces with titles/blurbs, bio,

...
- `instagram.txt` — caption for @juice.served (traditional work) or main
  feed (3D): hook first line, 2–3 short lines, then hashtag block
  (8–15 niche tags, not #art #love).
- `x.txt` — one post, under 260 chars, written like a peer in the
  archviz/UE5 community, not a marketer. Optionally a 2–3 post thread if
  there's a real process story.
- `shorts-script.md` — 15–30s short-form video script (if there's motion or
